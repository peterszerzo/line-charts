module LineChart.Axis.Tick exposing
    ( Config, int, float, time
    , long, gridless, labelless, opposite
    , custom, Properties, Direction, negative, positive
    , format, Time, Interval, Unit(..)
    )
...
format { zone, change, interval, timestamp, isFirst } =
    if isFirst then
        ...
        case change of
            Just change_ ->
                formatBold change_ zone timestamp

            Nothing ->
                formatNorm interval.unit zone timestamp
